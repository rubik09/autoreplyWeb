...
const incomingMessages = async (client, event) => {
    ...
    if (!chatId.length) await user.addUser(userId, apiId);
    ...
    for (const account of activeAccounts) {
        ...
        if (!statsArr.length) {
            ...
            await stats.addStats(count, stringifyKeywords, api_id, incomingMessages, averageMessagesCount);
            await statsSending(username, incomingMessages, count, averageMessagesCount, diffArr);
            await stats.updateClientStats(count ? count : 1, newKeywords[0].keywords, api_id, incomingMessages, averageMessagesCount);
            ...
            await statsSending(username, incomingMessagesStats, newUsersCount, averageMessagesCount, diffArr);
